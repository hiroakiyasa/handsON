...
- 3本の指定済み動画は初期値として登録済みで、SharePoint RESTからStream埋め込みURLを解決してページ内表示します。
- 埋め込み取得に失敗した場合も「動画を見る」ボタンから元のSharePoint動画を開けます。
...
次の動画URLはWebパーツの初期値へ登録済みです。

1. GitHub Copilotを用いたDRBFM作成紹介
2. PDFからテキスト抽出する紹介動画
3. Panel／INCAデータ解析エージェント

共有URLから動画ファイルのUniqueIdをSharePoint RESTで取得し、Streamの埋め込みプレーヤーとして各事例内に表示します。Graph APIは使用しません。既存ページへWebパーツを配置済みの場合でも、プロパティが未入力ならこの初期値が使用されます。
